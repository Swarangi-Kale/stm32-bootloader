...
const APP_HEADER_ADDR = (FLASH_BASE_ADDR + BOOTLOADER_SIZE)
const APP_HEADER_SIZE = (9 * 4) // 9 fields, each of 4 bytes

const VECTOR_TABLE_START_OFFSET = (0X80)

const FW_IMAGE_START_ADDR = (APP_HEADER_ADDR)
const APP_START_ADDR = (APP_HEADER_ADDR + VECTOR_TABLE_START_OFFSET)
const VECTOR_TABLE_SIZE = (0xb8)
const APP_VERIFICATION_FROM = (APP_START_ADDR + VECTOR_TABLE_SIZE)

const VALIDATION_START_OFFSET = VECTOR_TABLE_START_OFFSET + VECTOR_TABLE_SIZE; // 0x80 + 0xb8 = 0x138
...
const main = async () => {
  ...
  Logger.info('Updating firmware header');
  fwImage.writeUInt32LE(0x00000001, HEADER_OFFSETS.version);
  fwImage.writeUInt32LE(fwLength, HEADER_OFFSETS.fw_length);
  
  const validationLength = fwLength - VALIDATION_START_OFFSET;
  const crc = crc32(fwImage.subarray(VALIDATION_START_OFFSET), validationLength);
  Logger.info(`Computed crc value is: 0x${crc.toString(16).toUpperCase().padStart(8, '0')}`)
  // Logger.info(`${crc}`);
  fwImage.writeUInt32LE(crc, HEADER_OFFSETS.crc32);
